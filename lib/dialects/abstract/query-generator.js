...
class QueryGenerator {
    ...
    insertQuery(table, valueHash, modelAttributes, options) {
        ...
        if (this.dialect === "postgres") {
            let valueQuery = '<%= tmpTable %>INSERT INTO <%= table %> (<%= attributes %>)<%= output %> VALUES (<%= values %>)<%= ignoreDuplicates %>';
            let emptyQuery = '<%= tmpTable %>INSERT INTO <%= table %><%= output %><%= ignoreDuplicates %>';
        }
        let outputFragment;
